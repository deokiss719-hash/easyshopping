...
function createCommunityStore(pool, { imageUrlValidator = (value) => {
  ...
  async function listPosts({ category, q, scope = 'title', sort = 'latest', page = 1, size = 30, answer } = {}, viewerHash) {
    ...
    const order = sort === 'popular' ? `p.is_pinned DESC,p.is_notice DESC,${ranking} DESC,p.created_at DESC` : 'p.is_pinned DESC,p.is_notice DESC,p.created_at DESC,p.id DESC';
  ...
  async function popular(period='realtime',limit=5){
    ...
    return rows.map((row)=>{const ageHours=Math.max(0,(Date.now()-new Date(row.created_at).getTime())/3600000);const score=(Number(row.upvotes||0)*weights.up-Number(row.downvotes||0)*weights.down+Number(row.comment_count||0)*weights.comment+Math.min(Number(row.views||0),500)*weights.view)/Math.pow(2+ageHours,weights.age);return {row,score};}).sort((a,b)=>Number(Boolean(b.row.is_pinned))-Number(Boolean(a.row.is_pinned))||Number(Boolean(b.row.is_notice))-Number(Boolean(a.row.is_notice))||b.score-a.score||new Date(b.row.created_at)-new Date(a.row.created_at)).slice(0,Math.max(1,Math.min(20,Number(limit)||5))).map(({row})=>postRow(row));
  }
  async function adminListPosts({q='',answer='',deleted='active',page=1,size=50}={}){ const params=[]; const where=[]; if(deleted==='only')where.push('p.is_deleted=TRUE'); else if(deleted==='all')where.push('1=1'); else where.push('p.is_deleted=FALSE'); if(q){params.push(`%${String(q).trim().slice(0,100)}%`);where.push(`(p.title ILIKE $${params.length} OR p.body ILIKE $${params.length})`);} if(answer==='unanswered')where.push('p.answer_requested=TRUE AND p.answered_at IS NULL'); if(answer==='answered')where.push('p.answer_requested=TRUE AND p.answered_at IS NOT NULL'); const safeSize=Math.max(1,Math.min(100,Number(size)||50)); const safePage=Math.max(1,Number(page)||1); const countParams=[...params]; const count=await pool.query(`SELECT COUNT(*) total FROM community_posts p JOIN community_categories c ON c.id=p.category_id WHERE ${where.join(' AND ')}`,countParams); params.push(safeSize,(safePage-1)*safeSize); const {rows}=await pool.query(`SELECT p.*,c.slug category_slug,c.name category_name FROM community_posts p JOIN community_categories c ON c.id=p.category_id WHERE ${where.join(' AND ')} ORDER BY p.is_pinned DESC,p.is_notice DESC,p.updated_at DESC,p.id DESC LIMIT $${params.length-1} OFFSET $${params.length}`,params); return {posts:rows.map((r)=>({...postRow(r),hidden:r.is_hidden,deleted:r.is_deleted,authorKey:r.author_hash.slice(0,12),authorHash:r.author_hash})),total:Number(count.rows[0]?.total||0),page:safePage,size:safeSize}; }
  async function adminModeratePost(id,input){ const allowed=['show','hide','delete','restore','pin','unpin']; if(!allowed.includes(input.action))throw new TypeError('처리 방식을 확인해 주세요.'); if(input.action==='pin'||input.action==='unpin'){ const pinned=input.action==='pin'; const {rows}=await pool.query('UPDATE community_posts SET is_pinned=CASE WHEN is_notice=TRUE THEN TRUE ELSE $2 END,updated_at=CURRENT_TIMESTAMP WHERE id=$1 RETURNING id,is_hidden,is_deleted,is_pinned',[id,pinned]); return rows[0]||null; } const map={show:['FALSE','FALSE'],hide:['TRUE','FALSE'],delete:['TRUE','TRUE'],restore:['FALSE','FALSE']}; const [hidden,deleted]=map[input.action]; const {rows}=await pool.query(`UPDATE community_posts SET is_hidden=${hidden},is_deleted=${deleted},updated_at=CURRENT_TIMESTAMP WHERE id=$1 RETURNING id,is_hidden,is_deleted,is_pinned`,[id]); return rows[0]||null; }
  ...
  async function adminCreateNotice(input){ const title=safeText(input.title,160); const body=safeText(input.body,10000); const category=await pool.query("SELECT id FROM community_categories WHERE slug='free' AND is_active=TRUE LIMIT 1"); if(!category.rows[0])throw new TypeError('자유 카테고리를 찾을 수 없어요.'); const {rows}=await pool.query('INSERT INTO community_posts(category_id,title,body,nickname,author_hash,ip_display,is_notice,is_pinned,answer_requested) VALUES($1,$2,$3,$4,$5,$6,TRUE,TRUE,FALSE) RETURNING id',[category.rows[0].id,title,body,'이지핫딜 관리자','admin',null]); return getPost(rows[0].id,null,false); }
  ...
  return { categories,listPosts,getPost,createPost,updatePost,deletePost,createComment,deleteComment,vote,report,popular,publicSettings,sitemapPosts,adminListPosts,adminModeratePost,adminComments,adminModerateComment,adminCreateNotice,adminReply,adminReports,adminUpdateReport,adminSaveCategory,adminBannedWords,adminAddBannedWord,adminDeleteBannedWord,adminBlock,adminBlocks,adminDeleteBlock,settings,adminSaveSettings };
}
