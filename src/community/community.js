...
function postRow(row, viewerHash) {
  ...
  return {
    ...
    title: row.title, body: row.body, nickname: row.nickname, isNotice: Boolean(row.is_notice), answerRequested: row.answer_requested,
...
function createCommunityStore(pool) {
  ...
  async function listPosts({ category, q, scope = 'title', sort = 'latest', page = 1, size = 30, answer } = {}, viewerHash) {
    ...
    const order = sort === 'popular' ? `p.is_notice DESC,${ranking} DESC,p.created_at DESC` : 'p.is_notice DESC,p.created_at DESC,p.id DESC';
