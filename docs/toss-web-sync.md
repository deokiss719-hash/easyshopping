...
현재 토스 베스트 상품을 순위대로 최대 50개 웹 DB에 등록한다. 상위권에 안전하게 재사용할 수 있는 링크가 없는 상품이 있으면 다음 순위 상품으로 빈자리를 채운다. 평점 4.5 이상·리뷰 100개 이상이며 순위 30위 이내 또는 최근 24시간 사이트 클릭 3회 이상인 상품에는 인기 표시를 붙인다.
...
- 품절·현재 베스트 목록 제외·선택된 50개 밖 상품 종료
