...
카카오 자동발행 DB에서 `source=toss`, `status=sent`인 상품만 현재 토스 베스트 목록과 대조해 웹 DB에 등록한다. 기존 카카오 발행기 및 예약은 변경하지 않는다.

- 저장된 메시지 해시·발행 확인 시각·확인 증거·메시지 속 수익화 링크 검증
- 이미 발급된 `https://toss.im/_m/...` 링크 재사용. 신규 링크 발급 및 메시지 발송 없음
...
현재 토스 클릭/구매 실적의 집계 정상 여부는 이 연동과 별도로 검증해야 한다.
