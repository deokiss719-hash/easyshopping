...
test('admin login is public and noindex while management HTML remains authenticated', async (t) => {
  ...
  assert.match(adminScript, /summarizeReferrers/);
  assert.match(adminScript, /네이버/);
  assert.match(adminScript, /카카오·다음/);
  assert.match(adminScript, /row\.searchTerm/);
  assert.match(adminScript, /row\.referrerUrl/);
  assert.match(adminScript, /textContent/);
  assert.match(html, /id="community-notice-form"/);
  assert.match(html, /id="community-notice-list"/);
  assert.match(html, /공지글 등록/);
  ...
  assert.match(adminScript, /replyEditorOpen/);
  assert.match(adminScript, /community-admin-reply-form/);
  assert.match(adminScript, /!replyEditorOpen/);
